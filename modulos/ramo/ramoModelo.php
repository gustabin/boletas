...
if($option=="incluir"){ 
  ...
  if($_SESSION['token'] == $token){
      ...
      //incluir los datos en la tabla ramos
      $sql = "INSERT INTO `ramos` (`id`, `idsindicato`, `nombre`, `fecha`, `status`) VALUES (NULL, $idsindicato, '$nombre', NOW(), $status)";
...
if($option=="consultar"){
  ...
  if($_SESSION['token'] == $token){
      ...
      $sql = "SELECT * FROM ramos WHERE id = $clave";
...
if($option=="modificar"){  
  ...
  if($_SESSION['token'] == $token){ 
      ...
      while($data=mysqli_fetch_array($result))
        ...
        if(!empty($id))
        {
            $sql = "UPDATE ramos SET idsindicato = '$idsindicato', nombre = '$nombre', 
...
if($option=="eliminar"){
  ...
  if($_SESSION['token'] == $token){
      ...
      $sql = "DELETE FROM ramos WHERE id = $clave";
      if (mysqli_query($conn, $sql)) {
