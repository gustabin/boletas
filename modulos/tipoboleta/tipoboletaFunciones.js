...
function Incluir()  
{  
  document.querySelector('#titleModal').innerHTML = "Tipo de boleta";    
  ...
  if(localStorage.getItem("bandera") === null){
    ...
    $("body").on('submit', '#formDefault', function(event) { 
      ...
      if ($('#formDefault').valid()) {  
        $.ajax({
          type:"POST",
          url: "tipoboletaModelo.php?option=incluir",
          dataType: "json",
          data: $(this).serialize(),
          success: function(respuesta) {          
            if (respuesta.error == 1) {            
              swal("Houston, tenemos un problema", "Este tipo de boleta ya existe!", "error");          
            }
...
function Modificar(id,token) 
{
    ...
    {
      type:"POST",
      url: "tipoboletaModelo.php?option=modificarConsultar&id="+id+"&token="+token,
      dataType: "json",
      data: $(this).serialize(),
      success: function(respuesta) {
        
        if (respuesta.error == 1) {   
          swal("Houston, tenemos un problema", "Este tipo de boleta fue eliminado!", "error");                      
        }  
        if (respuesta.error == 2) {  
          swal("Houston, tenemos un problema", "Este tipo de boleta no fue encontrado!", "error");                
        }    
        if (respuesta.error == 3) {            
        ...
        if (respuesta.exito == 1) {          
          document.querySelector('#titleModal').innerHTML = "Actualizar tipo de boleta";
    ...
    $("body").on('submit', '#formDefault', function(event) 
    {
      ...
      if ($('#formDefault').valid()) {    
        $.ajax({
          type:"POST",
          url: "tipoboletaModelo.php?option=modificar",
          dataType: "json",
          data: $(this).serialize(),
          success: function(respuesta) {            
            if (respuesta.error == 1) {             
...
function Eliminar(id,token) {
    ...
    .then((willDelete) => {
      if (willDelete) {
        $.ajax({
            type: "POST",            
            url: "tipoboletaModelo.php?option=eliminar&id="+id+"&token="+token,
            dataType: "json",
            data: $(this).serialize(),
            success: function(respuesta) {              
              if (respuesta.error ==1) {                
              ...
              if (respuesta.error ==2) {                
                swal("Houston, tenemos un problema", "Este tipo de boleta está relacionado con un registro de otra tabla!, " + respuesta.errorDescription, "error");  
              }
              if (respuesta.exito == 1) {                       
                window.location.href='index.php';          
              }
