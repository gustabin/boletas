...
if($option=="incluir"){ 
  ...
  if($_SESSION['token'] == $token){
      ...
      $sql = "INSERT INTO `tipoboleta` (`id`, `idsindicato`, `nombre`, `fecha`, `status`) VALUES (NULL, $idsindicato, '$nombre', NOW(), $status)";
...
if($option=="consultar"){
  ...
  if($_SESSION['token'] == $token){
      ...
      $sql = "SELECT * FROM tipoboleta WHERE id = $clave";
      ...
      while($data=mysqli_fetch_array($result))
        ...
        if ($status==0)  //tipo boleta inactivo
        {                  
          $data = array("error" => '1');
          die(json_encode($data));
        }

        if ($status!=0)  //tipo boleta activo 
        {
...
if($option=="modificar"){  
  ...
  if($_SESSION['token'] == $token){ 
      ...
      while($data=mysqli_fetch_array($result))
        ...
        if(!empty($id))
        {
            $sql = "UPDATE tipoboleta SET idsindicato = '$idsindicato', nombre = '$nombre', 
...
if($option=="eliminar"){
  ...
  if($_SESSION['token'] == $token){
      ...
      $sql = "DELETE FROM tipoboleta WHERE id = $clave";
      if (mysqli_query($conn, $sql)) {
