...
function Incluir()   
{  
  document.querySelector('#titleModal').innerHTML = "Nueva categoría de empleados";    
  ...
  if(localStorage.getItem("bandera") === null){
    ...
    $("body").on('submit', '#formDefault', function(event) { 
      ...
      if ($('#formDefault').valid()) {  
        $.ajax({
          type:"POST",
          url: "categoriaempleadosModelo.php?option=incluir",
          dataType: "json",
          data: $(this).serialize(),
          success: function(respuesta) {          
            if (respuesta.error == 1) {            
              swal("Houston, tenemos un problema", "Esta categoría de empleados ya existe!", "error");          
            }
...
function Modificar(id,token) 
{
    ...
    {
      type:"POST",
      url: "categoriaempleadosModelo.php?option=modificarConsultar&id="+id+"&token="+token,
      dataType: "json",
      data: $(this).serialize(),
      success: function(respuesta) {
        
        if (respuesta.error == 1) {   
          swal("Houston, tenemos un problema", "Esta categoría de empleados fue eliminada!", "error");                      
        }  
        if (respuesta.error == 2) {  
        ...
        if (respuesta.exito == 1) {          
          document.querySelector('#titleModal').innerHTML = "Actualizar categoría de empleados";
    ...
    $("body").on('submit', '#formDefault', function(event) 
    {
      ...
      if ($('#formDefault').valid()) {    
        $.ajax({
          type:"POST",
          url: "categoriaempleadosModelo.php?option=modificar",
          dataType: "json",
          data: $(this).serialize(),
          success: function(respuesta) {            
            if (respuesta.error == 1) {             
...
function Eliminar(id,token) {
    ...
    .then((willDelete) => {
      if (willDelete) {
        $.ajax({
            type: "POST",            
            url: "categoriaempleadosModelo.php?option=eliminar&id="+id+"&token="+token,
            dataType: "json",
            data: $(this).serialize(),
            success: function(respuesta) {              
              if (respuesta.error ==1) {                
              ...
              if (respuesta.error ==5) {                
                swal("Houston, tenemos un problema", "Esta categoria está relacionada con un registro de otra tabla!, " + respuesta.errorDescription, "error");  
              }
              if (respuesta.exito == 1) {                       
                window.location.href='index.php';          
              }
